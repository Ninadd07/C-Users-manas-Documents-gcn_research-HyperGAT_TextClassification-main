        ...
        if isinstance(value, list):
            for item in value:
                if np.isnan(item).any():
                    print("NaN values found in keywords_dic:", key, item)

    # Save keywords_dic only if there are no NaN values
    if not any(np.isnan(item).any() for value in keywords_dic.values() for item in value):
        print("No NaN values found in keywords_dic. Saving to file.")
        pickle.dump(keywords_dic, open('trydata/' + dataset + '_LDA.p', "wb" ))
    else:
        print("NaN values found in keywords_dic. Not saving to file.")
